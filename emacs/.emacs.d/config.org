...
The interface is:
- ~C-c C-x C-i~, =org-clock-in=. Clock in on process at point and
...
Advice around =org-clock-in=. It should push an entry onto the stack if
it's different from the current entry on the top of the stack. 
#+begin_src emacs-lisp
...
[2023-11-27 Mon 23:37] Insert a uuid for each headline.
#+begin_src emacs-lisp
...
Recommended for interactive python [[https://elpy.readthedocs.io/en/latest/ide.html][here]].
#+begin_src emacs-lisp
...
*** cfg
:PROPERTIES:
:created:  2022-08-27 22:04:33 EDT
:header-args+: :noweb-ref elpy-config :tangle no
:END:
Needed to get xref-find-definitions working. See
https://github.com/jorgenschaefer/elpy/issues/948
#+begin_src emacs-lisp
(setq elpy-rpc-backend "jedi")
#+end_src

Have lines sent by elpy exist in python shell history for ~M-p~.
#+begin_src emacs-lisp
(setq elpy-shell-add-to-shell-history t)
#+end_src

Never echo python shell output in echo area.
#+begin_src emacs-lisp
...
Open the misc vterm buffer in same window, and cd to directory of
previous buffer.
#+begin_src emacs-lisp
...
Open vterm buffer (generally for temporary usage) below.
#+begin_src emacs-lisp
...
Open new vterm buffer same window.
#+begin_src emacs-lisp
...
*** ivy-rich
   ...
   :END:
[2022-08-26 Fri 19:42] Caused an error on loading emacs: error with
number of arguments passed to =(ivy-rich-set-display-transformer)=. I do
not recall the use of this section, so am removing it from tangling.
...
[2023-11-05 Sun 00:11] Gave some other themes a try. One bizarre thing
is that many themes don't seem to display the org mode modeline
clock. =manoj-dark= does. Using a slightly modified version now that
changes =ansi-color-blue=. 
#+BEGIN_SRC emacs-lisp 
  ;; (load-theme 'wombat)
